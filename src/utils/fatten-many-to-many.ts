...
type InnerOf<T, R extends keyof T, N extends keyof PivotItem<T, R>> = PivotItem<T, R>[N];
...
>(rows: T[], relationKey: R, nestedKey: N, outKey: OutKey, dedupeBy?: keyof InnerOf<T, R, N> & string) {
  ...
  return rows.map((row) => {
    ...
    const normalized =
      dedupeBy && items.length
        ? Array.from(new Map((items as any[]).map((it) => [it?.[dedupeBy], it])).values())
        : items;
